...
```bash
cargo run --bin formula-screening -- \
...
- ルートの `Cargo.toml` から参照される Rust ソース置き場
...
- `peg.py`: Trailing PEG（`peg_trailing`）と独自ブレンドPEG（`peg_blended_2f`）を計算する。いずれもEPSベース（`stock_db` の `compute_eps` で計算済み）。
  ...
  - `peg_trailing_with_status()` / `peg_blended_2f_with_status()` は数値に加えて `ok` / `missing_input` / `insufficient_history` / `non_positive_per` / `non_positive_eps` / `non_positive_growth` を返す。UI は `non_positive_growth` を欠損と区別して `neg` 表示する。

### `src/formula_screening/web.py`

- `/api/screening` と `/api/stock-price-meta` を返す API ルートを作る
- `stock_web_ui` の `serve()` に `docs/assets`、`IndexPage`、API ルートを渡す
- handbook 参照用に `../japan_company_handbook/data` を `yazi_base_dir` として渡す
- 外部利用向けの `compute_all_stock_metrics(db_path=...)` は Rust binding `formula_screening._core` を呼び、呼び出し側が指定した DB だけを読んで `has_preferred_shares` も返す
- Python `stock` 辞書向けの `create_screening_api()` / `save_screening_json()` と、Rust payload 向けの `create_screening_payload_api()` / `save_screening_payload_json()` を持つ。`save_stock_price_metadata_json()` は GitHub Pages 用に最新株価日付を `docs/assets/stock-price-meta.json` へ保存する。

### CLI 出力挙動

- `screen` サブコマンドはスクリーニング実行後、結果が 0 件でも常に `docs/assets/screening.json` と `docs/assets/stock-price-meta.json`（GitHub Pages 用）を自動生成する
...
- ローカル時は `/api/screening`、GitHub Pages 時は `assets/screening.json` を fetch する。株価基準日は `/api/stock-price-meta` または `assets/stock-price-meta.json` から取得してステータス欄に表示する
...
`peg_trailing_5_status` / `peg_blended_5y_actual_2f_status` は対応する PEG 数値の状態を表す文字列です。数値フィールドは従来どおり nullable のまま維持し、負成長などの未算出理由は status 側で区別します。

株価基準日は payload 本体には混ぜず、ローカル API では `/api/stock-price-meta`、静的配信では `assets/stock-price-meta.json` から `{ "price_date": "YYYY-MM-DD" }` として取得します。

PyO3 の互換 API として `run_screening_payload_py()` は従来どおり payload 配列だけを返します。通常 CLI は `run_screening_payload_with_diagnostics_py()` を使い、`payload` に加えて `diagnostics` を受け取ります。`diagnostics` は全スクリーニング対象銘柄を対象に、`code`, `name`, `missing_fields`, `unavailable_fields` を返します。PEG の `missing_input` / `insufficient_history` は `missing_fields` と `unavailable_fields` の両方に載り、`non_positive_per` / `non_positive_eps` / `non_positive_growth` は `unavailable_fields` に理由付きで載ります。不正な優先株フラグを持つ非ヒット銘柄がいても payload 生成自体は継続します。
...
CLI の既定 DB パスは `path.toml` ではなく `stock_db.paths.STOCKS_DB_PATH` を使用します。外部 API 呼び出しでは `db_path` を明示でき、PyO3 経路と standalone Rust CLI の指標期間はどちらも `magic_numbers.toml` を読む。  
