...
`screen` の結果には、戦略定義の有無にかかわらず共通リンク列 `monex` / `sikiho` が自動で追加される。標準出力では固定幅テーブルを表示し、OSC 8 ハイパーリンク対応端末ではリンク列を直接クリックできる。非対応端末や非 TTY 出力では plain text ラベルにフォールバックする。`--output` の CSV には同じ列名で生 URL を書き出す。戦略側で `COLUMNS` や `columns(stock)` を定義している場合は、その後ろに共通リンク列が連結される。
